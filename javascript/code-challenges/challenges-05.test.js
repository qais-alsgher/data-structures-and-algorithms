...
------------------------------------------------------------------------------------------------ */
const toLastNames = people =>people.map(ele=>`${ele.firstName} ${ele.lastName}`);
...
const addValues = (arr) =>arr.reduce((total,current)=>{
...
const returnNames = (arr) => {
  const arrName=arr.reduce((allNmae,current)=>{
    return [...allNmae,current.name];
  },[]);
  return arrName;
};
...
const reversN =str.split('').reduce((previusChar,currentChar)=>{
    return currentChar+previusChar;
  },'')
  return reversN;
};
...
const countNumberOfChildren = (arr) => {
  let numChild;
const allChildren=arr.reduce((allChild,current)=>{
 if(current.children){
 numChild=allChild+=current.children.length;
  }
 return numChild;
},0)
return numChild;
...
const calculateAverage = (arr) => {
  let count=0;
  let sum=0;
const avgNum= arr.reduce((prev,current)=>{
sum=prev+current;
count+=1;
return sum;
},0)
...
const countPrimeNumbers = (arr) => {
  let numPrim=0;
  let counter=arr.reduce((prev,current)=>{
    if(isPrime(current)){
      numPrim +=1;
    }
    return numPrim;
