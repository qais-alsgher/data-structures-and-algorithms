...
const raisedToTheThird = (arr) => {
  // Solution code here...
  let power=[];
  arr.forEach(element=>power.push(Math.pow(element,3)));
  return power;
};
...
const addOne =arr=>arr.map((ele)=>ele+1);
...
const addQuestion =arr=>arr.map((ele)=>ele.concat('?'));
...
const charCode = (arr) =>arr.map((ele)=>ele.charCodeAt(0));
