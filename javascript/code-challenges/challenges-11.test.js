...
function transformToLis(obj){
  ...
  for(let i=0;i<keyObj.length;i++){
    ...
    arrHtml.push(`<li>${keyObj[i]}: ${valueObj[i]}</li>`);
  }
...
const count = (target, input) => {
  let cont=0;
  let result=input.map(ele=>{
    return(ele.filter(items=>{
      return items===target?cont++:0;
    }))
  })
...
const totalSum = (input) => {
  ...
  let sum=input.map(ele=>{
    return(
      ele.reduce((prev,current)=>{
        return prev+current;
      },0)
    )
  })

let totleSum=sum.reduce((pev,current)=>{
  return pev+current;
},0)
return totleSum;
};
...
let arr=input.map(ele=>{
  return(
    ele.filter(items=>{
      if(items%5===0 && typeof(items)==='number'){
       return items;
      }
    })
  )
...
let result=arr.map(ele=>{
  return(
    ele.reduce((prev,current)=>{
     return[...prev, Math.pow(2, current)];
    },[])
  )
})
  return result;
...
let findMaleAndFemale = (data) => {
  let arrGnd=data.filter(items=>{
    if(items.gender==='male'||items.gender==='female')
  return items;
})


let arrName=arrGnd.reduce((prev,current,index)=>{
return index===0?`${current.name}`:`${prev} and ${current.name}`;
},"")
...
Write a function named findShortest that, given the Star Wars data from Challenge 5, uses any combination of filter, map and reduce to return the name of the character who is the shortest in height.
------------------------------------------------------------------------------------------------ */

let findShortest = (data) => {

let m=data.reduce((prev,current)=>{
return [...prev,current.name];
},[])
let min=m[0];
for(let i=0;i<m.length;i++){
  if(min.length===m[i].length){
    min=m[i];
  }else if(min.length>m[i].length){
    min=m[i];
  }
}
return min;
