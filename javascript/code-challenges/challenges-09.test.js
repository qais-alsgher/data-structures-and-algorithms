...
const checkValues = (obj, value) => {
   ...
   let find=Object.values(obj).map((ele)=>{
  if(ele===value){
  return true;
  }else{
    return false;
...
let allData=[];
let dataArr=Object.keys(obj);
let dateArr=Object.values(obj);
for(let i=0;i<dataArr.length;i++){
allData.push(`${dataArr[i]}: ${dateArr[i]}`);
...
for (let i = 0; i <valArr.length; i++) {
  if (valArr[i].name===character) {
    if (valArr[i].children) {
      result=true;
    }
  }
}
